                                ...
                                for (let indxxxxxx = 0; indxxxxxx < users.length; indxxxxxx++) {
                                    ...
                                    if (userrr.unique_id === tow_driver_id) {
                                        ...
                                        "data": {
                                            ...
                                            const calculateService = (service) => {
                                                switch (service) {
                                                    case "door-unlocking":
                                                        return company_informtion.services.unlock_locked_door_cost;
                                                        break;
                                                    case "gas-delivery":
                                                        return company_informtion.services.gas_delivery_cost;
                                                        break;
                                                    case "tire-change": 
                                                        return company_informtion.services.change_tire_cost;
                                                        break;
                                                    case "stuck-vehicle":
                                                        return company_informtion.services.remove_stuck_vehicle;
                                                        break;
                                                    case "jump-start":
                                                        return company_informtion.services.jumpstart_cost;
                                                        break;
                                                    default:
                                                        break;
                                                }
                                            ...
                                            for (let indxxxxxxxx = 0; indxxxxxxxx < user.card_payment_methods.length; indxxxxxxxx++) {
                                                ...
                                                if (cardddd.primary === true) {
                                                    ...
                                                    axios.get(`${config.get("ngrok_url")}/gather/company/related/information`, {
                                                        ...
                                                        const paymentIntent = await stripe.paymentIntents.create({
                                                            payment_method_types: ['card'],
                                                            amount: Math.round((calculateService(roadside_service)) * 100),
                                                            currency: 'usd',
                                                            application_fee_amount: Math.round(Math.round((calculateService(roadside_service)) * 100) * 0.23),
                                                            transfer_data: {
                                                              destination: stripe_account_id,
                                                            },
                                                            customer: user.stripe_customer_account.id,
